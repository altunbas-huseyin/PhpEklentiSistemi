<?php
	...
	function add_action( $tag = null , $function = null )
	{
		global $hooks;
		$hooks->add_action($tag , $function);
	}
	
	function do_action( $tag = null , $parameters = null )
	{
		global $hooks;
		if( !is_null($parameters) )
			$hooks->do_action( $tag , $parameters );
		$hooks->do_action( $tag );
	}
	
	function apply_filters( $tag , $variable = null )
	{
		global $hooks;
		
		if( !is_null($variable) )
		{
			$return = $hooks->apply_filters($tag , $variable);
		}
		else
		{
			$return = $hooks->apply_filter($tag , $variable );
		}
		
		return $return;
	}
	
	function add_filter( $tag = null , $function = '' )
	{
		global $hooks;
		$hooks->add_filter( $tag , $function );
	}
	
	function remove_filter( $tag = null , $function = '' )
	{
		global $hooks;
		$hooks->remove_filter( $tag , $function );
	}